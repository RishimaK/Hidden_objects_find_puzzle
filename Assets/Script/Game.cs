...
public class Game : MonoBehaviour
{
    ...
    private SaveDataJson.Map MapData;
    ...
    public GameObject CollectionIC;
    public GameObject ListCardIC;
    ...
    public void SetChallengeData (SaveDataJson.Challenge data)
    {
        ChallengeData = data;
    ...
    // public void TakeImagesFromResources() => ListItem = Resources.LoadAll<Sprite>("Items"); 

    // public Sprite TakeImageItem(string name)
    // {
    //     Sprite itemNeed = null;
    //     foreach (Sprite item in ListItem){
    ...
    private void SetValue()
    {
        ...
        if(!CollectionIC.activeSelf && findedItem >= 40)
        {
    ...
    void CheckItemInMap()
    {
        string[] itemsFinded = (string[]) saveDataJson.GetData($"ItemMap{MapNum}");
        string[] itemsNeedToShow = (string[]) saveDataJson.GetData($"ShowHiddenItems{MapNum}");
    ...
    {
        collectionInfo.SetCardImage(MapNum);
        string[] collection = (string[]) saveDataJson.GetData("Collection");
        if(collection == null || collection.Length == 0) return;
        int count = 0;
        foreach(string child in collection)
        {
            if(Convert.ToInt32(child.Split("_")[0]) == MapNum)
            {
                Transform card = ListCard.transform.Find(child.Split("_")[1]);
                card.gameObject.SetActive(false);
                collectionInfo.SetImage(child.Split("_")[1]);
                count++;
            }
        }
        if(count == 4)
    ...
    public void LoadGame(int mapNum) 
    {
        ...
        MapNum = mapNum;
        ListCard = GameObject.Find($"Game/MainGame/Map/Map{mapNum}/ListCard");
        ListToFind = GameObject.Find($"Game/MainGame/Map/Map{mapNum}/AvailableItemList");
        ListItemFinded = GameObject.Find($"Game/MainGame/Map/Map{mapNum}/ListItemFinded");
        HiddenItemList = GameObject.Find($"Game/MainGame/Map/Map{mapNum}/HiddenItemList");
        SoulFireList = GameObject.Find($"Game/MainGame/Map/Map{mapNum}/SoulFifeList");
        ...
        if(mapNum >= 2 || (int)saveDataJson.GetData("OpenedMap") >= 2)
        {
            ListCard.SetActive(true);
            CollectionIC.SetActive(true);
        }
        ...
        StartCoroutine(ChangeNameArea(MapData.AreaName));
        ...
        CheckSoulFifeList();
        checkLimited();
        cloudAnimation.PlayChangeScreenAnimation("Start");
    }
    ...
    public void AfterChangeScreenAnimation()
    {
        ...
        // if(MapNum >= 3 || (int)saveDataJson.GetData("OpenedMap") >= 3)
    ...
    void CloudAnimation(int areaNum)
    {
        ...
        for(int i = 0; i < 3; i++)
        {
            ...
            // if(i == 0) clouds.GetChild(i).GetComponent<SkeletonAnimation>().AnimationState.Complete += AnimationCompleteHandler;
            if(i == 0) Invoke("AnimationCompleteHandler", 1.167f);
        }
    }

    // void AnimationCompleteHandler(TrackEntry trackEntry)
    void AnimationCompleteHandler()
    {
        ...
        else
        {
            ...
            beTouched = true;
            buttonControl.OnBtn();
            if (MapNum == 2 && (int)saveDataJson.GetData("OpenedMap") == 2 && ((string[])saveDataJson.GetData($"ItemMap{MapNum}")).Length == 0)
            {
                compassTxt.transform.parent.gameObject.SetActive(true);
                PlayCompassTutorial();
            }
    ...
    IEnumerator CountTimeTakeItem(int currentTime = 90)
    {
        ...
        if (hintBtn.gameObject.activeSelf)
        {
            if (MapNum <= 2 && (int)saveDataJson.GetData("OpenedMap") <= 2)
            {
                int hintValue = (int)saveDataJson.GetData("Hint");
                if (hintValue == 0)
                {
                    hintValue++;
                    saveDataJson.SaveData("Hint", hintValue);
                    hint.text = $"{hintValue}";
                    shop.hintText.text = $"{hintValue}";
                }
            }
...
#region Tutorial
    ...
    void Drop()
    {
        ...
        {
            if(item.transform.parent.name == "ListCard")
            {
                PlayCollectCardAnimation(item);
            }
            else if(item.transform.parent.name == "SoulFifeList")
            {
                FoundSoulFire(item);
            }
            else if(item.transform.parent.name != "Content")
            {
                ...
                currentTotalItem += 1;
                if(currentTotalItem == 40 && MapNum == 1 && (int)saveDataJson.GetData("OpenedMap") == 1)
                {
                    PlayListCardTutorial();
                }
    ...
    void HandleItemAfterTouch(dragable item)
    {
    ...
    IEnumerator EvaluateSlerpPoints(dragable item, GameObject card, Vector3 start, Vector3 path, Vector3 end, string txt = "") {
        if(txt == "start") interpolateAmount = 0;
        yield return new WaitForSeconds(Time.deltaTime);
        interpolateAmount += Time.deltaTime * 2.5f;

        card.transform.position = QuadraticLerp(start, path, end, interpolateAmount);

        // Debug.Log(card.transform.position + " / " + end);
        if(card.transform.position != end) StartCoroutine(EvaluateSlerpPoints(item, card, start, path, end));
        else 
        {
            StartCoroutine(RemoveParticle(card.transform));
            CollectionIC.transform.DOScale(new Vector3(0.8f,0.8f,0.8f), 0.2f).SetEase(Ease.OutQuad).OnComplete(() => {
                collectionInfo.SetImage(item.name);
                CollectionIC.transform.DOScale(new Vector3(1f,1f,1f), 0.2f).SetEase(Ease.InQuad);
                collectionInfo.CheckImage();
                if(MapNum == 1 && item.name == "1") PlayListCardTutorial2();
            });
            card.gameObject.SetActive(false);
        }
    }
    ...
    void PlayCollectCardAnimation(dragable item)
    {
        item.gameObject.SetActive(false);
        saveDataJson.SaveData("Collection", $"{MapNum}_{item.name}");
        home.GetComponent<Home>().OpenNewCollection(MapNum, Convert.ToInt32(item.name));
        if(item.name == "1") CollectionIC.transform.GetChild(1).gameObject.SetActive(false);
        // CollectionIC.SetActive(true);
        Transform card = ListCardIC.transform.GetChild(0);
        foreach(Transform child in ListCardIC.transform)
        {
        ...
        card.GetComponent<Image>().sprite = controlImage.TakeImage(controlImage.ListCard, $"{MapNum}_{item.name}");
        card.gameObject.SetActive(true);
        card.GetComponent<RectTransform>().localScale = new Vector2 (2f, 2f);
        AddParticleToItem(card.gameObject);
        card.GetChild(0).GetComponent<Transform>().localPosition = new Vector2(0, -172/2);
        card.position = item.transform.position;

        CollectionIC.transform.DOPause();
        // rect.localScale = new Vector2(1,1);

        card.DOScale(new Vector3(0.7f, 0.7f, 0.5f), 1f);
        // rect.DOAnchorPos(new Vector2(85, -217), 0.5f).OnComplete(() => {
            float tb = (card.position.y + CollectionIC.transform.position.y) / 2;

            Vector3[] path = new Vector3[3];
            path[0] = card.position;
            path[1] = card.position.x >= 0 ? new Vector3(card.position.x - 2.5f, tb, card.position.z) : new Vector3(card.position.x + 2.5f, tb, card.position.z);
            path[2] = CollectionIC.transform.position;
            audioManager.PlaySFX("collect_item");
            StartCoroutine(EvaluateSlerpPoints(item, card.gameObject, path[0], path[1], path[2], "start"));

            // rect.DOAnchorPos(new Vector2(-50, -217), 0.5f).SetDelay(2).OnComplete(() => {
                // CollectionIC.SetActive(false);
            // });
        // });
    }
    ...
    void FoundSoulFire (dragable item)
    {
        ...
        SkeletonAnimation itemAnimation = item.GetComponent<SkeletonAnimation>();
        itemAnimation.AnimationState.SetAnimation(0, "animation", false);
        // itemAnimation.AnimationState.TimeScale = 0.5f;
        item.transform.DOScale(new Vector3(1.2f,1.2f,1), 0.5f).OnComplete(() => {
            item.transform.DOScale(new Vector3(0,0,1), 0.5f).SetDelay(1f).OnComplete(() => {
                ...
                itemAnimation.AnimationState.SetAnimation(0, "idle", true);
    ...
    {
        GameObject[] listItemAvailable = {};
        // Vector2 camSize = canvas.GetComponent<RectTransform>().sizeDelta * canvas.GetComponent<RectTransform>().localScale / 2;
        // Debug.Log(camSize);
        // camSize -= new Vector2(mapGame.transform.position.x, mapGame.transform.position.y);

        // Vector3 pos = new Vector3(3.75f, 6.675f, mapGame.transform.position.z);
        // if(areaNum == 1) pos = new Vector3(-3.75f, 6.675f, mapGame.transform.position.z);
        // else if(areaNum == 2) pos = new Vector3(3.75f, -6.675f, mapGame.transform.position.z);
        // else if(areaNum == 3) pos = new Vector3(-3.75f, -6.675f, mapGame.transform.position.z);
        if(gameObject.GetComponent<Game>().enabled)
        {
            int[] Milestones = MapData.Milestones;
            int areaMap = 1;
            ...
            {   
                ...
                Vector3 childPos = child.transform.position - mapGame.transform.position;
                // if(rightLimit + camSize.x >= childPos.x && childPos.x >= leftLimit - camSize.x &&
                //     upperLimit + camSize.y >= childPos.y && childPos.y >= bottomLimit - camSize.y)
                // {
        ...
        {
            for(int i = 0; i < ChallengeItem.transform.childCount; i++)
            {   
                Transform child = ChallengeItem.transform.GetChild(i);
                if(child.gameObject.activeSelf){
                    Array.Resize(ref listItemAvailable, listItemAvailable.Length + 1);
                    listItemAvailable[listItemAvailable.Length - 1] = child.gameObject;
                }
    ...
    void ShowCompleteDilog()
    {
        ...
        string[] showHiddenItems = (string[])saveDataJson.GetData($"ShowHiddenItems{MapNum}");
